...
@Component({
  selector: 'app-leave-approval-inline',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './leave-approval-inline.html',
...
export class LeaveApprovalInline implements OnInit {
  ...
  private get baseUrl(): string {
    return this.role === 'vp'
      ? `${BASE}/vp/dashboard/leave-requests`
      : `${BASE}/admin/dashboard/leave-requests`;
  }
  ...
  reject(id: number): void {
    this.http.patch(`${this.baseUrl}/${id}/reject`, { reason: 'Rejected' }, { headers: this.headers })
      .pipe(catchError(() => of(null)))
      .subscribe(r => { if (r !== null) this.load(); });
  }
