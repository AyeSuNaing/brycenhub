...
interface HistoryResponse {
  ...
  totalRecords: number;
  draftCount: number;
  hrReviewedCount: number;       // reused as PENDING_APPROVAL count
...
export class PayrollHistoryInline implements OnInit {
  ...
  load(payPeriod?: string) {
    ...
    this.http.get<HistoryResponse>(`${PAYROLL_BASE}/history${qs}`,
      ...
      .subscribe({
        next: data => {
          ...
          this.loading = false;
          if (this.selectedPeriod) this.loadBatchStatus();
          this.cdr.detectChanges();
        },
        error: err => {
  ...
  loadBatchStatus() {
    ...
    this.http.get<BatchStatus>(
      `${PAYROLL_BASE}/batch-status?branchId=${this.branchId}&payPeriod=${this.selectedPeriod}`,
      { headers: this.auth.getHeaders() })
      .subscribe({
        next: data => {
  ...
  onPeriodChange() {
    if (this.selectedPeriod) this.load(this.selectedPeriod);
  }
  ...
  doBatchAction() {
    ...
    const url = this.actionDialog === 'submit'
      ? `${PAYROLL_BASE}/batch/submit`
      : `${PAYROLL_BASE}/batch/mark-paid`;
    ...
    this.http.post<any>(url, {
      ...
      .subscribe({
        next: _ => {
          const wasPaid = this.actionDialog === 'paid';  // ← null set မလုပ်ခင် check
          this.actionLoading = false;
          this.actionDialog  = null;
          this.actionNote    = '';
          this.load(this.selectedPeriod);
        
          // 🔔 Mark as Paid → Announcement + Notification တစ်ခါတည်း refresh
          if (wasPaid) {
