...
public class AdminDashboardController {
	...
	public ResponseEntity<List<OtRequestDto.Response>> getOtRequests(@AuthenticationPrincipal User admin,
			@RequestParam(defaultValue = "PENDING") String status) {

		Long branchId = admin.getBranchId();
		boolean isComp = isCompanyAdmin(admin);

		List<OtRequest> list = isComp ? otRequestRepository.findByStatusOrderByCreatedAtDesc(status)
	...
	public ResponseEntity<?> rejectOt(@PathVariable Long id,
			@RequestBody(required = false) OtRequestDto.RejectRequest body, @AuthenticationPrincipal User admin) {
		try {
			OtRequest ot = otRequestRepository.findById(id)
					.orElseThrow(() -> new RuntimeException("OT request not found"));
	...
	public ResponseEntity<List<LeaveRequestDto.Response>> getLeaveRequests(@AuthenticationPrincipal User admin,
			@RequestParam(defaultValue = "PENDING") String status) {

		Long branchId = admin.getBranchId();
		boolean isComp = isCompanyAdmin(admin);

		List<LeaveRequest> list = isComp ? leaveRequestRepository.findByStatusOrderByCreatedAtDesc(status)
	...
	public ResponseEntity<?> rejectLeave(@PathVariable Long id,
			@RequestBody(required = false) LeaveRequestDto.RejectRequest body, @AuthenticationPrincipal User admin) {
		try {
			LeaveRequest lv = leaveRequestRepository.findById(id)
					.orElseThrow(() -> new RuntimeException("Leave request not found"));
	...
	public ResponseEntity<List<LeaveRequestDto.TodayLeaveResponse>> getTodayLeave(@AuthenticationPrincipal User admin) {
